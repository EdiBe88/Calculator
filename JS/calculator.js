...
const Calculator = {
    ...
    First_Operand: null,
    //this checks whether the operator, we set it to null for now
    operator: null,
} ;
...
function Handle_Operator(Next_Operator) {
    ...
    //checks if an operaator already exist and if Wait_Second_Operand is true,
    ...
    } else if (operator) {//checks if an operator alreadyexists
...
keys.addEventListener('click', (event) => {
    //the target variable is an object that represents the eement
    //that was clicked
    const { target } = event;
